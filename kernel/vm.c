...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    // only the writeable page need COW
    if((*pte & PTE_W) != 0){
      *pte &= ~PTE_W; // avoid write for copy on write
    ...
    kref((void *)pa); // increase the reference num
  }
  return 0;
...
// check the access bit of page table
int
pgaccess(void *base, int len, void *mask)
{
  struct proc *p = myproc();
  uint64 va = (uint64)base, ans = 0;
  for(int i = 0; i < len; i++, va += PGSIZE) {
    pte_t *pte = walk(p->pagetable, va, 0);
    if(*pte & PTE_A) {
      ans |= 1 << i;
    }
    *pte = (((*pte) & (1 << 6)) ^ (*pte)) ^ (0 << 6); // clear the access bit
  }
  if(copyout(p->pagetable, (uint64)mask, (char *)&ans, sizeof(ans)) < 0) {
    return -1;
  }
  return 0;
}

// deal with the copy on write trap
int
...
#ifdef LAB_MMAP
// deal with the mmap trap
int
mmap(struct proc *p, uint64 va) {
  if (va >= MMAPED || va < MMAPBG) {
    return -1;
  }
  int id = (va - MMAPBG) / PGSIZE;
